...
<p style="color:#5A6478;font-size:12px;text-align:center;margin:0;">
  ℹ️ TI foi copiado neste e-mail apenas para acompanhamento.</p>"""
    ...
    html = _base_template(subject, content)
    send_email(rafael_email, subject, html, cc=["ti@rezendeenergia.com.br"])
...
<table cellpadding="0" cellspacing="0" width="100%">
  <tr><td style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);
    ...
    html = _base_template(subject, content)
    send_email(sol.solicitante_email, subject, html, cc=[
        "RezendeRH@rezendeenergia.com.br",
        "RezendeDP@rezendeenergia.com.br",
        "RezendeSESMT@rezendeenergia.com.br",
        "ti@rezendeenergia.com.br",
...
def notify_solicitacao_gestor(sol, base_url: str):
    """Confirma para o gestor que sua solicitação foi recebida — CC: RH, DP, TI, rh@."""
    ...
    for lbl, val in campos:
        rows_html += (
            '<tr>'
            '<td style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);'
            'border-radius:8px;padding:10px 16px;width:38%;font-size:11px;color:rgba(255,106,0,0.8);'
            'font-weight:700;text-transform:uppercase;letter-spacing:1.5px;">' + lbl + '</td>'
            '<td style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.04);'
            'border-radius:8px;padding:10px 16px;color:#F4F5F7;font-size:14px;">' + (val or "—") + '</td>'
            '</tr>'
        )
    ...
    content = (
        ...
        'letter-spacing:2px;font-weight:600;">Aguardando aprovação da diretoria</p>'
        '<p style="color:#A8A8B8;font-size:15px;line-height:1.7;margin:0 0 16px;">'
        'Olá, <strong style="color:#fff;">' + sol.solicitante_nome + '</strong>! '
        'Sua solicitação foi recebida e encaminhada para aprovação da diretoria. '
        'Você será notificado assim que houver uma decisão.</p>'
        '<table cellpadding="0" cellspacing="0" width="100%" '
